...
std::tuple<bool, bool> Dialog_ChangePartyMember::draw() {
  ...
  if (ImGui::BeginPopupModal(std::format("{}###ChangePartyMember", m_dialogId).c_str(), &m_open, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize)) {
    drawPickers();
