...
std::tuple<bool, bool> DBTermsTab::drawAbbreviatedString(const float width, std::string_view label, std::string& full, std::string& abbrv) {
                    ...
                    ImGuiChildFlags_FrameStyle | ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_AlwaysAutoResize);
  {
    ...
    if (ImGui::LabelOverLineEdit(std::format("##{}_term", label).c_str(), trNOOP("Full"), levelStr, 256, width, nullptr, ImGuiInputTextFlags_None)) {
      full = levelStr;
      fullModified = true;
    }
    ImGui::SameLine();
    ...
    if (ImGui::LabelOverLineEdit(std::format("##{}_abbreviation_term", label).c_str(), trNOOP("Abbreviation"), abvrStr, 256, width, nullptr, ImGuiInputTextFlags_None)) {
      abbrv = abvrStr;
      abbrvModified = true;
    }
    ImGui::PopStyleColor();
...
bool DBTermsTab::drawString(const float width, const std::string_view label, std::string& full) {
                    ...
                    ImGuiChildFlags_FrameStyle | ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_AlwaysAutoResize);
  {
    ...
    char levelStr[256];
    strncpy(levelStr, full.c_str(), 256);
    if (ImGui::LabelOverLineEdit(std::format("##{}_term", label).c_str(), label.data(), levelStr, 256, width, nullptr, ImGuiInputTextFlags_None)) {
      full = levelStr;
      result = true;
    }
    ImGui::PopStyleColor();
