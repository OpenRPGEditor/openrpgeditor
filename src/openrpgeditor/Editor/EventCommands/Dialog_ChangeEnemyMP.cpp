...
std::tuple<bool, bool> Dialog_ChangeEnemyMP::draw() {
  ...
  if (ImGui::BeginPopupModal(std::format("{}###ChangeEnemyMP", m_dialogId).c_str(), &m_open, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize)) {
    drawPickers();
    ImGui::BeginVertical("##change_enemy_mp_main_layout", ImGui::GetContentRegionAvail());
    {
      GroupBox enemyGroupBox(trNOOP("Enemy"), "##change_enemy_mp_enemy_group", {-1, 0}, nullptr, ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_AlwaysAutoResize);
      ...
      GroupBox operationGroupBox(trNOOP("Operation"), "##change_enemy_mp_operation_group", {-1, 0}, nullptr,
                                 ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_AlwaysAutoResize);
      if (operationGroupBox.begin()) {
        ImGui::BeginHorizontal("##change_enemy_mp_operation_group_layout", {ImGui::GetContentRegionAvail().x, -1});
        {
          ImGui::Spring(0.33);
      ...
      GroupBox operandGroupBox(trNOOP("Operand"), "##change_enemy_mp_operand_group", {-1, 0}, nullptr, ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_AlwaysAutoResize);
      if (operandGroupBox.begin()) {
        ImGui::BeginHorizontal("##change_enemy_mp_operand_group_layout", {-1, -1});
        {
          ImGui::BeginVertical("##change_enemy_mp_operand_group_source_layout", {-1, -1});
          {
            ImGui::RadioButton("Constant", &m_quantitySource, 0);
            ImGui::RadioButton("Variable", &m_quantitySource, 1);
          }
          ImGui::EndVertical();
          ImGui::BeginVertical("##change_enemy_mp_operand_group_value_layout", {-1, -1});
          {
            ImGui::BeginDisabled(m_quantitySource != 0);
            ImGui::SetNextItemWidth(-1);
            if (ImGui::SpinInt("##change_enemy_mp_constant", &m_quantity, 1, 100, m_quantitySource == 0 ? nullptr : "")) {
              m_quantity = std::clamp(m_quantity, 1, 9999);
            }
            ImGui::EndDisabled();

            ImGui::BeginDisabled(m_quantitySource != 1);
            ImGui::PushID("##change_enemy_mp_quant_var");
      ...
      ImGui::BeginHorizontal("##change_enemy_mp_buttons_layout", {-1, -1});
      {
        ImGui::Spring();
        if (const auto ret = ImGui::ButtonGroup("##change_enemy_mp_buttons", {trNOOP("OK"), trNOOP("Cancel")}); ret == 0) {
          m_confirmed = true;
...
void Dialog_ChangeEnemyMP::drawPickers() {
  if (m_variablePicker) {
