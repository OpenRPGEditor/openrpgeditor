...
struct Dialog_ChangeEnemyMP : IEventDialogController {
  ...
  explicit Dialog_ChangeEnemyMP(const std::string& name, const std::shared_ptr<ChangeEnemyMPCommand>& cmd = nullptr, const int troopId = 0)
  : IEventDialogController(name)
  , m_troopId(troopId)
  , m_command(cmd) {
    if (cmd == nullptr) {
      m_command.reset(new ChangeEnemyMPCommand());
    }
...
private:
  ...
  std::shared_ptr<ChangeEnemyMPCommand> m_command;
};