...
void MovementRoute::setList(const std::vector<std::shared_ptr<IEventCommand>>& list) {
  ...
  m_list = list;
  if (!signalsDisabled()) {
    emit_signal(listModified(), this, list);
  }
  setModified();
}
...
void MovementRoute::setRepeat(bool repeat) {
  ...
  m_repeat = repeat;
  if (!signalsDisabled()) {
    emit_signal(repeatModified(), this, repeat);
  }
  setModified();
}
...
void MovementRoute::setSkippable(bool skippable) {
  ...
  if (!signalsDisabled()) {
    emit_signal(skippableModified(), this, skippable);
  }
  setModified();
}
...
void MovementRoute::setWait(bool wait) {
  ...
  m_wait = wait;
  if (!signalsDisabled()) {
    emit_signal(waitModified(), this, wait);
  }
  setModified();
}
