...
void DBStatesTab::draw() {
  ...
  {
    ImGui::BeginChild("##orpg_states_editor_states", ImVec2{250.f, 0} * App::DPIHandler::get_ui_scale(), 0,
                      ImGuiWindowFlags_HorizontalScrollbar);
    {
      ImGui::BeginGroup();
      {
        ImGui::SeparatorText("States");
        ImGui::BeginChild("##orpg_states_editor_States_list",
                          ImVec2{0, ImGui::GetContentRegionMax().y - (App::DPIHandler::scale_value(108))});
        {
          ImGui::BeginGroup();
          {
            for (auto& skill_ : m_states.states()) {
              ...
              if (ImGui::Selectable(name, &skill_ == m_selectedState) ||
                  (ImGui::IsItemFocused() && m_selectedState != &skill_)) {
