...
std::tuple<bool, bool> Dialog_ChangeHP::draw() {
  ...
  if (ImGui::BeginPopupModal(std::format("{}###ChangeHP", m_dialogId).c_str(), &m_open, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize)) {
    drawPickers();
        ...
        ImGui::BeginHorizontal("##change_hp_actor_group_horizontal", {-1, -1});
        {
          ...
          ImGui::BeginVertical("##change_hp_actor_group_buttons", {-1, -1});
          {
            ...
            if (ImGui::EllipsesButton(m_comparison == 1 ? Database::instance()->variableNameAndId(m_valueVar).c_str() : "", {-1, 0})) {
              m_isOperand = false;
              m_variablePicker.emplace(VariableSwitchPicker::Type::Variable, Database::instance()->system->variables(), m_valueVar);
      ...
      if (operandGroupBox.begin()) {
        ImGui::BeginHorizontal("##change_hp_operand_horizontal", {-1, -1});
        {
          ...
          ImGui::BeginVertical("##change_hp_operand_buttons", {-1, -1});
          {
            ...
            if (ImGui::SpinInt("##change_hp_operand_constant", &m_quantity, 1, 100, m_quantitySource == 0 ? nullptr : "")) {
              if (m_quantity > 9999)
                m_quantity = 9999;
              if (m_quantity < 1)
                m_quantity = 1;
