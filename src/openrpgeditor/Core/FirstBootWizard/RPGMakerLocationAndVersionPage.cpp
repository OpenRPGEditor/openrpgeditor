...
void RPGMakerLocationAndVersionPage::draw() {
  ...
  if (ImGui::LabelOverLineEdit("##rpgmaker_location_line_edit", "RPG Maker Location(*)", location, sizeof(location), 0.f)) {
  ...
  ImGui::BeginGroup();
  {
    ...
    if (ImGui::Button("Choose...")) {
      nfdu8char_t* loc;
      const auto result = NFD_PickFolder(&loc, !Settings::instance()->rpgMakerLocation.empty() ? Settings::instance()->rpgMakerLocation.c_str() : nullptr);
      if (result == NFD_OKAY) {
  ...
  {
    ImGui::Text("Version(*)");
    if (ImGui::BeginCombo("##rpgmaker_version_combo", Settings::instance()->rpgMakerVersion != -1 ? KnownRPGMVVersions[Settings::instance()->rpgMakerVersion].data() : "Please select a version")) {
  ...
  m_allRequiredFieldsAreSet = std::filesystem::is_directory(Settings::instance()->rpgMakerLocation) && Settings::instance()->rpgMakerVersion != -1;
}
