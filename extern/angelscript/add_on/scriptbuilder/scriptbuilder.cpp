...
asIScriptEngine* CScriptBuilder::GetEngine() { return engine; }

asIScriptModule* CScriptBuilder::GetModule() { return module; }

unsigned int CScriptBuilder::GetSectionCount() const { return (unsigned int)(includedScripts.size()); }
...
int CScriptBuilder::BuildModule() { return Build(); }
...
int CScriptBuilder::ExtractDeclaration(int pos, string& name, string& declaration, int& type) {
  ...
  do {
    ...
    t = engine->ParseToken(&modifiedScript[pos], modifiedScript.size() - pos, &len);
    token.assign(&modifiedScript[pos], len);
  } while (t == asTC_WHITESPACE || t == asTC_COMMENT || token == "private" || token == "protected" || token == "shared" || token == "external" || token == "final" || token == "abstract");
...
string GetAbsolutePath(const string& file) {
  ...
  if (!((str.length() > 0 && (str[0] == '/' || str[0] == '\\')) || str.find(":") != string::npos)) {
