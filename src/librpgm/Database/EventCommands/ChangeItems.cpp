...
std::string ChangeItemsCommand::stringRep(const Database& db, const bool colored) const {
  ...
  return indentText(indent()) + symbol(code()) + ColorFormatter::getColorCode(code(), colored) + trNOOP("Change Items") + colon.data() + itemName + " " + DecodeEnumName(operation) + " " + operandStr +
