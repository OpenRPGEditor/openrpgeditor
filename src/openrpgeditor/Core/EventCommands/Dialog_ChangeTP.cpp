...
std::tuple<bool, bool> Dialog_ChangeTP::draw() {
  ...
  if (ImGui::BeginPopupModal(std::format("{}###ChangeTP", m_name).c_str(), &m_open, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize)) {
    drawPickers();
    ImGui::BeginVertical("##change_tp_main_layout", ImGui::GetContentRegionAvail());
    {
      GroupBox actorGroupBox(trNOOP("Actor"), "##change_tp_actor_group", {-1, 0}, nullptr, ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_AlwaysAutoResize);
      if (actorGroupBox.begin()) {
        ImGui::BeginHorizontal("##change_tp_actor_group_horizontal", {-1, -1});
        {
          ImGui::BeginVertical("##change_tp_actor_group_radios", {-1, -1});
          {
          ...
          ImGui::EndVertical();
          ImGui::Spring();
          ImGui::BeginVertical("##change_tp_actor_group_buttons", {-1, -1});
          {
      ...
      actorGroupBox.end();
      GroupBox operationGroupBox(trNOOP("Operation"), "##change_tp_operation_group", {-1, 0}, nullptr, ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_AlwaysAutoResize);
      if (operationGroupBox.begin()) {
        ImGui::BeginHorizontal("##change_tp_operation_group_horizontal", {ImGui::GetContentRegionAvail().x, -1});
        {
          ImGui::Spring(.5);
      ...
      GroupBox operandGroupBox(trNOOP("Operand"), "##change_tp_operand", {-1, 0}, nullptr, ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_AlwaysAutoResize);
      if (operandGroupBox.begin()) {
        ImGui::BeginHorizontal("##change_tp_operand_horizontal", {-1, -1});
        {
          ImGui::BeginVertical("##change_tp_operand_radios", {-1, -1});
          {
          ...
          ImGui::EndVertical();
          ImGui::Spring();
          ImGui::BeginVertical("##change_tp_operand_buttons", {-1, -1});
          {
            ImGui::BeginDisabled(m_quantitySource != 0);
            ImGui::SetNextItemWidth(-1);
            if (ImGui::SpinInt("##change_tp_operand_constant", &m_quantity, 1, 100, m_quantitySource == 0 ? "%d" : "")) {
              if (m_quantity > 100)
                m_quantity = 100;
              if (m_quantity < 1)
                m_quantity = 1;
      ...
      ImGui::BeginHorizontal("##change_tp_dialog_buttons", {-1, -1});
      {
        ImGui::Spring();
        if (const auto ret = ImGui::ButtonGroup("##change_tp_dialog_buttons", {trNOOP("OK"), trNOOP("Cancel")}); ret == 0) {
          m_confirmed = true;
...
void Dialog_ChangeTP::drawPickers() {
  if (m_actorPicker) {
