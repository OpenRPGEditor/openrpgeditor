...
std::tuple<bool, bool> Dialog_ChangeEnemyTP::draw() {
  ...
  if (ImGui::BeginPopupModal(std::format("{}###ChangeEnemyTP", m_dialogId).c_str(), &m_open, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize)) {
    drawPickers();
    ImGui::BeginVertical("##change_enemy_tp_main_layout", ImGui::GetContentRegionAvail());
    {
      ...
      GroupBox operationGroupBox(trNOOP("Operation"), "##change_enemy_tp_operation_group", {-1, 0}, nullptr,
                                 ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_AlwaysAutoResize);
      if (operationGroupBox.begin()) {
        ImGui::BeginHorizontal("##change_enemy_tp_operation_group_layout", {ImGui::GetContentRegionAvail().x, -1});
        {
          ImGui::Spring(0.33);
      ...
      GroupBox operandGroupBox(trNOOP("Operand"), "##change_enemy_tp_operand_group", {-1, 0}, nullptr, ImGuiChildFlags_AutoResizeX | ImGuiChildFlags_AutoResizeY | ImGuiChildFlags_AlwaysAutoResize);
      if (operandGroupBox.begin()) {
        ImGui::BeginHorizontal("##change_enemy_tp_operand_group_layout", {-1, -1});
        {
          ImGui::BeginVertical("##change_enemy_tp_operand_group_source_layout", {-1, -1});
          {
            ImGui::RadioButton("Constant", &m_quantitySource, 0);
            ImGui::RadioButton("Variable", &m_quantitySource, 1);
          }
          ImGui::EndVertical();
          ImGui::BeginVertical("##change_enemy_tp_operand_group_value_layout", {-1, -1});
          {
            ImGui::BeginDisabled(m_quantitySource != 0);
            ImGui::SetNextItemWidth(-1);
            if (ImGui::SpinInt("##change_enemy_tp_constant", &m_quantity, 1, 100, m_quantitySource == 0 ? nullptr : "")) {
...
void Dialog_ChangeEnemyTP::drawPickers() {
  if (m_variablePicker) {
