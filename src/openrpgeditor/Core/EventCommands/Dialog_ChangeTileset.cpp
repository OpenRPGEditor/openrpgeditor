...
std::tuple<bool, bool> Dialog_ChangeTileset::draw() {
  ...
  ImGui::SetNextWindowSize(ImVec2{300, 95}, ImGuiCond_Appearing);
  if (ImGui::BeginPopupModal(m_name.c_str(), &m_open, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    if (ImGui::Button((std::format("{:04}", m_tileset) + Database::instance()->tilesetName(m_tileset)).c_str(), ImVec2{(280), 0})) {
