...
bool ColorFlashPicker::draw() {
  ...
  ImGui::BeginGroup();
  {
                          ...
                          ImGuiChildFlags_FrameStyle);
    {
      ...
      win->DrawList->AddImage(m_tonePreview, win->ContentRegionRect.Min,
                              win->ContentRegionRect.Min + ImVec2{static_cast<float>(m_tonePreview.width()),
                                                                  static_cast<float>(m_tonePreview.height())});
      win->DrawList->AddRectFilled(win->ContentRegionRect.Min,
                                   win->ContentRegionRect.Min + ImVec2{static_cast<float>(m_tonePreview.width()),
                                                                       static_cast<float>(m_tonePreview.height())},
