...
std::tuple<bool, bool> Dialog_ChangeState::draw() {
  ...
  if (ImGui::BeginPopupModal(std::format("{}###ChangeState", m_dialogId).c_str(), &m_open, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize)) {
    drawPickers();
