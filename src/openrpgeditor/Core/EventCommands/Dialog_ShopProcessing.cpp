  ...
  if (ImGui::BeginPopupModal(m_name.c_str(), &m_open, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    if (ImGui::BeginTable("##shop_processing_main", 2, ImGuiTableFlags_Borders | ImGuiTableFlags_RowBg | ImGuiTableFlags_ScrollX | ImGuiTableFlags_ScrollY, ImVec2{530, 390})) {
      ...
      if (ImGui::TableNextColumn()) {
        std::string text = m_type == 2 ? Database::instance()->armorName(m_id) : m_type == 0 ? Database::instance()->itemName(m_id) : Database::instance()->weaponName(m_id);
        const bool isSelected = (m_goods_selection == 0);
        if (ImGui::SelectableWithBorder(text.c_str(), isSelected, ImGuiSelectableFlags_AllowOverlap | ImGuiSelectableFlags_SpanAllColumns | ImGuiSelectableFlags_AllowDoubleClick)) {
