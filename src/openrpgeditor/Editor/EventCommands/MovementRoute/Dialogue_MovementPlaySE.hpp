...
struct Dialog_MovementPlaySE : IEventDialogController {
  Dialog_MovementPlaySE() = delete;
  explicit Dialog_MovementPlaySE(const std::string& name, const std::shared_ptr<MovementPlaySECommand>& cmd = nullptr)
  : IEventDialogController(name)
  , command(cmd) {
    if (cmd == nullptr) {
      command.reset(new MovementPlaySECommand());
    }
    m_audio = command->se;
...
private:
  ...
  std::shared_ptr<MovementPlaySECommand> command;
