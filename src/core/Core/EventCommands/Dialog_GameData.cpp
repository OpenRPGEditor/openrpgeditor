...
void Dialog_GameData::draw() {
  ...
  if (ImGui::BeginPopupModal(m_name, &m_open, ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoScrollbar)) {
    ...
    ImGui::SameLine();
    std::string text =
        d_source != 0 ? "##gamedata_item_empty" : (d_raw_source == 0 ? "" : std::format("{:04} ", d_raw_source) + m_project->item(d_raw_source)->name);
    ...
    ImGui::SameLine();
    text =
        d_source != 1 ? "##gamedata_weapon_empty" : std::format("{:04} ", d_raw_source) + m_project->weapon(d_raw_source)->name;
    ...
    ImGui::SameLine();
    text =
        d_source != 2 ? "##gamedata_armor_empty" : std::format("{:04} ", d_raw_source) + m_project->armor(d_raw_source)->name;
    ...
    ImGui::SameLine();
    text =
        d_source != 3 ? "##gamedata_actor_empty" : std::format("{:04} ", d_raw_source) + m_project->actor(d_raw_source)->name;
    ...
    if (ImGui::BeginCombo("##gamedata_actor_combo", ""))
    {
      for (auto dataSource : actorSourceNames) {
        ...
        if (ImGui::Selectable(dataSource, is_selected))
    ...
    if (ImGui::BeginCombo("##gamedata_character_combo", ""))
    {
      for (auto dataSource : characterSourceNames) {
        bool is_selected = (current_characterSource == dataSource);
        if (ImGui::Selectable(dataSource, is_selected))
          current_characterSource = dataSource;
        if (is_selected)
          ImGui::SetItemDefaultFocus();
      }
    }
