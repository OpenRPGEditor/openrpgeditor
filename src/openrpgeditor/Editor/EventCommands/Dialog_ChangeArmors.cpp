...
std::tuple<bool, bool> Dialog_ChangeArmors::draw() {
  ...
  if (ImGui::BeginPopupModal(std::format("{}###ChangeArmors", m_dialogId).c_str(), &m_open, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_AlwaysAutoResize)) {
    drawPickers();
