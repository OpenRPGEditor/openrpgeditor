...
void DBWeaponsTab::draw() {
  ...
  {
    ImGui::BeginChild("##orpg_weapons_editor_weapons", ImVec2{250.f, 0} * App::DPIHandler::get_ui_scale(), 0,
                      ImGuiWindowFlags_HorizontalScrollbar);
    {
      ImGui::BeginGroup();
      {
        ImGui::SeparatorText("Weapons");
        ImGui::BeginChild("##orpg_weapons_editor_weapons_list",
                          ImVec2{0, ImGui::GetContentRegionMax().y - (App::DPIHandler::scale_value(108))});
        {
          ImGui::BeginGroup();
          {
            for (auto& skill_ : m_weapons.weapons()) {
              ...
              if (ImGui::Selectable(name, &skill_ == m_selectedWeapon) ||
                  (ImGui::IsItemFocused() && m_selectedWeapon != &skill_)) {
