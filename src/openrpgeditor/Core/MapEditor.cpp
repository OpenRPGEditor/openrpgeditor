...
void MapEditor::drawParallax(ImGuiWindow *win) {
  ...
  win->DrawList->AddImage(m_parallaxTexture, win->ContentRegionRect.Min + ImVec2{0.f, 0.f},
                          win->ContentRegionRect.Min + (ImVec2{
                                                          static_cast<float>(m_parallaxTexture.width()),
...
void MapEditor::drawGrid(ImGuiWindow *win) {
  for (int y = tileSize() * m_mapScale; y < (map()->height() * tileSize()) * m_mapScale; y += tileSize() * m_mapScale) {
    win->DrawList->AddLine(win->ContentRegionRect.Min + ImVec2{0.f, static_cast<float>(y)},
                           win->ContentRegionRect.Min + ImVec2{
                             (map()->width() * tileSize()) * m_mapScale, static_cast<float>(y)
                           },
                           0x7f0a0a0a, 3.f);
  }
  ...
  for (int x = tileSize() * m_mapScale; x < (map()->width() * tileSize()) * m_mapScale; x += tileSize() * m_mapScale) {
    win->DrawList->AddLine(win->ContentRegionRect.Min + ImVec2{static_cast<float>(x), 0.f},
                           win->ContentRegionRect.Min + ImVec2{
                             static_cast<float>(x), (map()->height() * tileSize()) * m_mapScale
                           },
                           0x7f0a0a0a, 3.f);
  }
}
...
void MapEditor::handleEventDrag() {
  if (m_movingEvent) {
    ...
    const auto it =
        std::ranges::find_if(map()->events(), [&](const std::optional<Event> &e) {
...
void MapEditor::handleMouseInput(ImGuiWindow *win) {
  ...
  if (m_selectedEvent && !m_movingEvent && m_selectedEvent->x() != m_tileCursor.tileX() && m_selectedEvent->y() !=
      m_tileCursor.tileY()) {
  ...
  if (ImGui::IsMouseClicked(ImGuiMouseButton_Left) && ImGui::IsWindowHovered() && m_parent->editMode() ==
      EditMode::Event && m_movingEvent == nullptr) {
    m_selectedEvent = map()->eventAt(m_tileCursor.tileX(), m_tileCursor.tileY());
    m_movingEvent = m_selectedEvent;
    if (m_movingEvent) {
      m_movingEventX = m_movingEvent->x();
  ...
  if ((ImGui::IsMouseDoubleClicked(ImGuiMouseButton_Left) || ImGui::IsKeyPressed(ImGuiKey_Enter) ||
       ImGui::IsKeyPressed(ImGuiKey_KeypadEnter)) && m_parent->editMode() == EditMode::Event &&
  ...
  if ((ImGui::IsMouseClicked(ImGuiMouseButton_Left) || ImGui::IsMouseDragging(ImGuiMouseButton_Left)) && m_parent->
  ...
  if (((m_tileCursor.mode() == MapCursorMode::Keyboard && (ImGui::IsKeyReleased(ImGuiKey_LeftShift)) ||
        ImGui::IsKeyReleased(ImGuiKey_RightShift)) || ImGui::IsMouseReleased(ImGuiMouseButton_Left)) &&
...
void MapEditor::updateShadowData(const Point &point) {
  const int tileId = (isGroundTile(point, 0) && isShadowingTile(Point{point.x() - 1, point.y()}, 0) && isShadowingTile(
                        Point{point.x() - 1, point.y() - 1}, 0))
                       ? 5
                       : 0;
...
int MapEditor::updateFloorTypeAutotile(const Point &point, const int layer, const int flags) const {
  ...
  const auto newShape = TileHelper::floorDirToShape(
    rightBottom | rightTop | leftTop | leftBottom | leftMiddle | topMiddle | rightMiddle | bottomMiddle | (
      dir & ~flags));
...
int MapEditor::updateWallTypeAutotile(const Point &point, const int layer, const int flags) const {
  ...
  if (isSameKindTile(Point{xCoord - 1, yCoord}, layer, tileId)) {
    hasSameKindLeft = !isSameKindTile(Point{xCoord - 1, tmpYStart}, layer, tileId) || !isSameKindTile(
                        Point{xCoord - 1, tmpYEnd}, layer, tileId);
  }

  if (isSameKindTile(Point{xCoord + 1, yCoord}, layer, tileId)) {
    hasSameKindRight = !isSameKindTile(Point{xCoord + 1, tmpYStart}, layer, tileId) || !isSameKindTile(
                         Point{xCoord + 1, tmpYEnd}, layer, tileId);
  }
  ...
  if (TileHelper::isWallSideTile(tileId)) {
    if (!hasSameKindLeft) {
      hasSameKindLeft = !isWallOrRoofTile(Point{xCoord - 1, yCoord}, layer) || !
                        isWallOrRoofTile(Point{xCoord - 1, tmpYStart}, layer) || !isWallOrRoofTile(
                          Point{xCoord - 1, tmpYEnd}, layer);
    }
...
void MapEditor::draw() {
  ...
  if (ImGui::Begin(trNOOP("Map Editor"), nullptr, ImGuiWindowFlags_HorizontalScrollbar | ImGuiWindowFlags_NoTitleBar)) {
    ImGui::BeginChild("##mapcontents",
                      ImVec2(
                        0, ImGui::GetContentRegionAvail().y - (
                             ImGui::CalcTextSize("S").y + (ImGui::GetStyle().FramePadding.y * 2) + ImGui::GetStyle().
                             ItemSpacing.y)), 0,
    ...
    if (map()) {
      ...
      ImGui::Dummy(ImVec2{
        (map()->width() * Database::instance()->system.tileSize() * m_mapScale),
        (map()->height() * Database::instance()->system.tileSize() * m_mapScale)
      });

      const float u1 = std::clamp((static_cast<float>(map()->width() * tileSize()) * m_mapScale) / (8192 * 2), 0.f,
                                  1.f);
      const float v1 = std::clamp((static_cast<float>(map()->height() * tileSize()) * m_mapScale) / (8192 * 2), 0.f,
                                  1.f);

      win->DrawList->AddImage(static_cast<ImTextureID>(m_checkeredBack), win->ClipRect.Min + ImVec2{0, 0},
                              win->ClipRect.Min + (ImVec2{
                                                     static_cast<float>(map()->width() * tileSize()),
                                                     static_cast<float>(map()->height() * tileSize())
                                                   } * m_mapScale), ImVec2{0, 0},
                              ImVec2{u1, v1});
      ...
      if (ImGui::BeginPopupContextWindow()) {
        ...
        if (ImGui::MenuItem(trNOOP("Insert..."))) {
          ...
          template_picker = ObjectPicker(
            trNOOP("Templates"), Database::instance()->templates.templateList(Template::TemplateType::Event), 0);
          ...
          if (confirmed) {
            ...
            if (m_templateSaving) {
              ...
              if (template_picker.value().selection() == 0) {
                Database::instance()->templates.addTemplate(Template(
                  Database::instance()->templates.templates.size() + 1,
                ...
                Database::instance()->templates.templates.at(template_picker.value().selection() - 1).setCommands(
                  eventJson.dump());
                templateName_picker = TemplateName(
                  &Database::instance()->templates.templates.at(template_picker.value().selection() - 1), nullptr);
              }
              Database::instance()->templates.templates.back().setCommands(eventJson.dump());
            } else {
              nlohmann::ordered_json eventJson = nlohmann::ordered_json::parse(
                Database::instance()->templates.templates.at(template_picker->selection() - 1).commands());
        ...
        if (templateName_picker->hasChanges()) {
          if (Database::instance()->templates.serialize(Database::instance()->basePath + "data/Templates.json")) {
            ImGui::InsertNotification(ImGuiToast{
              ImGuiToastType::Success, trNOOP("Saved event as template successfully!")
            });
          } else {
      ...
      for (const auto &event: sortedEvents) {
        ...
        if (m_selectedEvent == event && !m_hasScrolled) {
          ImGui::SetScrollX(
            (win->ContentRegionRect.Min.x / 2) + (
              ((event->x() * tileSize()) * m_mapScale) - (win->ContentRegionRect.Max.x / 2)));
          ImGui::SetScrollY(
            (win->ContentRegionRect.Min.y / 2) + (
    ...
    ImGui::EndChild();
    ImGui::BeginChild("##map_editor_bottom_panel", ImVec2{}, 0,
      ...
      std::string fmt = trFormat("Tile {}, ({}, {})",
                                 m_mapRenderer.tileId(m_tileCursor.tileX(), m_tileCursor.tileY(), 0),
                                 m_tileCursor.tileX(), m_tileCursor.tileY());
      if (map()) {
        if (const auto ev = std::ranges::find_if(map()->events(), [&](const std::optional<Event> &e) {
...
double MapEditor::adjustX(double x) {
  return x + ((ImGui::GetCurrentWindow()->ContentRegionRect.Min.x / tileSize()) * m_mapScale);
}

double MapEditor::adjustY(double y) {
  return y + ((ImGui::GetCurrentWindow()->ContentRegionRect.Min.y / tileSize()) * m_mapScale);
}
...
double MapEditor::xWithDirection(const double x, const Direction d) {
  return x + (d == Direction::Right ? 1 : d == Direction::Left ? -1 : 0);
}

double MapEditor::yWithDirection(const double y, const Direction d) {
  return y + (d == Direction::Down ? 1 : d == Direction::Up ? -1 : 0);
}

double MapEditor::roundXWithDirection(const double x, const Direction d) {
  return roundX(x + (d == Direction::Right ? 1 : d == Direction::Left ? -1 : 0));
}

double MapEditor::roundYWithDirection(const double y, const Direction d) {
  return roundY(y + (d == Direction::Down ? 1 : d == Direction::Up ? -1 : 0));
}
